...
namespace Swashbuckle.Swagger
{
    public class SwaggerGenerator : ISwaggerProvider
    {
        ...
        public SwaggerDocument GetSwagger(string rootUrl, string apiVersion)
        {
            ...
            var pathss = GetApiDescriptionsFor(apiVersion);
            var pathsss = pathss.Where(apiDesc => !(_options.IgnoreObsoleteActions && apiDesc.IsObsolete()));
            var path3 = pathsss.OrderBy(_options.GroupingKeySelector, _options.GroupingKeyComparer);
            var path4 = path3.GroupBy(apiDesc => apiDesc.RelativePathSansQueryString());
            var paths = path4.ToDictionary(group => "/" + group.Key, group => CreatePathItem(group, schemaRegistry));
